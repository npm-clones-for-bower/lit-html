...
const keyToIndexMapCache = new WeakMap();
    ...
    return directive((containerPart) => {
        ...
        while (oldHead <= oldTail && newHead <= newTail) {
            if (oldHeadPart === null) {
                ...
                newTailKey = newKeys[--newTail];
                oldTailKey = oldKeys[--oldTail];
                oldTailPart = oldParts[oldTail];
            }
            else if (!newKeyToIndexMap.has(oldHeadKey)) {
                // Old head is no longer in new list; remove
                removePart(oldHeadPart);
                oldHeadKey = oldKeys[++oldHead];
                oldHeadPart = oldParts[oldHead];
            }
            else if (!newKeyToIndexMap.has(oldTailKey)) {
                ...
                newParts[newTail] = updatePart(oldHeadPart, newResults[newTail]);
                ...
                newParts[newHead] = updatePart(oldTailPart, newResults[newHead]);
                ...
                const oldIndex = oldKeyToIndexMap.get(newHeadKey);
...
//# sourceMappingURL=repeat-references.js.map